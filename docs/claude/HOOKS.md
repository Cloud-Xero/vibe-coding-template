...
このドキュメントでは、HTML/CSS、JavaScript/TypeScript（React含む）、Google Apps Script、Go、Astro プロジェクト向けに最適化された Claude Code Hooks の設定について詳しく解説します。

## 概要

Claude Code Hooks は開発プロセスの重要な段階で自動的にチェックを実行し、コード品質とセキュリティを確保するシステムです。この設定では以下の技術スタックに対応しています：
...
### pre_commit フック
コミット前に実行される軽量なチェック群。開発者の生産性を損なわない範囲で基本的な品質を確保します。

### pre_push フック
プッシュ前に実行される包括的なテスト・セキュリティチェック。時間はかかりますが、より徹底した検証を行います。

### post_checkout フック
ブランチ切り替えやクローン後に実行される環境セットアップ。依存関係のインストールや設定ファイルの準備を自動化します。

### pre_deploy フック
デプロイ前の最終検証。本番環境への影響を最小限に抑えるための厳格なチェックを実行します。

### post_deploy フック
デプロイ後の動作確認。アプリケーションが正常に稼働しているかを検証します。
...
### 1. 設定ファイルの配置
`.claude/settings.json` にフック設定を配置します。
